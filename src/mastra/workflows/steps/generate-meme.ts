...
export const generateMemeStep = createStep({
    id: 'generate-meme',
    description: 'Create a meme using ImgFlip API',
    ...
    execute: async({inputData}) => {
        try{
            ...
            if(!result.success)
                throw new Error(`Imgflip API error while adding caption ${result.error_message}`)
            ...
            console.log('Unable to generate meme')
            throw new Error('Unable to generate meme')
        }
    }
